...
console.log("PASS", process.env.MONGO_PASSWORD);
...
const defaultHomePost2 = new Post({
  title: postLog[0].title,
  body: postLog[0].body,
});
const defaultHomePost3 = new Post({
  title: postLog[1].title,
  body: postLog[1].body,
});

const defaultHomePost = [defaultHomePost2, defaultHomePost3];
// defaultHomePost.save();
...
app.get("/", (req, res) => {
  ...
  Post.find({}, (err, foundItems) => {
    if (foundItems.length == 0) {
      Post.insertMany(defaultHomePost, (err) => {
        if (!err) {
...
app.get("/post/:postID", (req, res) => {
  ...
  Post.findById(req.params.postID, (err, foundedPosts) => {
    hasMatch = true;
    var temp = String(foundedPosts._id.str);

    console.log(temp);
    res.render('post.ejs', {
      customPostTitle: foundedPosts.title,
      customPostBody: foundedPosts.body,
    });
  })
